...
import datetime
import json
import sysfrom pyspark import SparkConf, SparkContextclass AverageSpeedEnhancer(object):
    ...
    output_schema = [  # This is the schema of nyc-tlc:yellow.trips
        ...
        with closing(cStringIO.StringIO()) as csv_string:
            ...
            return csv_string.getvalue().strip()def enhance_with_avg_speed(self, record):
        """
        ...
        if record.get('store_and_fwd_flag') and record.get('store_and_fwd_flag') not in 'YN':
            record['store_and_fwd_flag'] = Noneif (record['pickup_datetime'] and record['dropoff_datetime']
            ...
            record['average_speed'] = Nonecsv_record = self.dict_to_csv(record)
        return csv_recorddef main(sc, gcs_path_raw, gcs_path_transformed):
    ...
    transformed_records_rdd.saveAsTextFile(gcs_path_transformed)if __name__ == '__main__':spark_conf = SparkConf()
    spark_conf.setAppName('AverageSpeedEnhancement')
    spark_context = SparkContext(conf=spark_conf)parser = argparse.ArgumentParser()parser.add_argument('--gcs_path_raw', dest='gcs_path_raw',
                        required=True,
                        help='Specify the full GCS wildcard path to the json files to enhance.')parser.add_argument('--gcs_path_transformed', dest='gcs_path_transformed',
                        required=True,
                        help='Specify the full GCS path prefix for the transformed json files. ')
    known_args, _ = parser.parse_known_args(None)main(sc=spark_context, gcs_path_raw=known_args.gcs_path_raw,
